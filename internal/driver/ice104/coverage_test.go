...
func TestDecoderAllTypeIDs(t *testing.T) {
	...
	pts, err := dec.DecodeInformationObject(typeM_SP_NA_1, []byte{0x01}, false, 1)
	require.NoError(t, err)
	require.Len(t, pts, 1)
	...
	_, err = dec.DecodeInformationObject(999, []byte{0x01}, false, 1)
	require.Error(t, err)
}
...
func TestSendGeneralCallAndSingleCommand(t *testing.T) {
	...
	assert.Equal(t, startByte, written[0])
}
