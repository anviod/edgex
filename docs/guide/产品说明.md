...
所有驱动统一使用公共连接管理器 (ConnectionManager)，提供工业级可靠性保障：

**核心特性：**
- **统一状态机**：`Disconnected → Connecting → Connected → Retrying → Dead` 五态管理
- **指数退避**：`backoff = min(base_delay × 2^retry_count, max_delay) + jitter`
- **冷却期策略**：基础冷却 1 分钟，指数增长，最大 1 小时，防止持续打设备
- **采集健康检测**：采集成功 = 连接健康，取消独立心跳，减少设备负载
- **低频补偿探测**：采集周期超过 3 倍时自动触发轻量探测
- **每日清零**：每日零点自动重置重试计数与冷却次数

**适用驱动**：S7、Modbus、EtherNet/IP、OPC UA、FINS、SNMP、DL/T645、Profinet IO、KNXnet/IP（BACnet 采用故障隔离机制）
...
> 完整驱动矩阵与测试覆盖见 [设备驱动](../drivers/index.html) · [南向驱动测试报告](../testing/南向驱动测试报告.html)

| 协议 | 注册名 | 状态 | 说明 |
| :--- | :--- | :--- | :--- |
| **Modbus TCP / RTU / RTU Over TCP** | `modbus-tcp`, `modbus-rtu`, `modbus-rtu-over-tcp` | 生产就绪 | 基于 simonvetter/modbus；智能 MTU 探测、指数退避、24h 冷却期 |
...
| **IEC 60870-5-104** | `iec60870-5-104` | M1 已交付 | 总召唤、自发上报、单点遥控；ConnectionManager 开发中 |
| **DL/T645-2007** | `dlt645` | 已实现 | 基于协议实现；17 项单元测试 |
| **Mitsubishi SLMP** | `mitsubishi-slmp` | 生产就绪 | MC Protocol；7 项单元测试 |
| **Profinet IO** | `profinet-io` | 已实现 | DCP/RPC I/O 模块读写；本地网口绑定；6 项单元测试 |
| **KNXnet/IP** | `knxnet-ip` | 生产就绪 | 网关发现；组地址读写；TCP/UDP 双模式；10 项单元测试 |
...
| **OPC UA 服务端** | 已交付 | 基于 awcullen/opcua 实现，支持多种认证方式，支持双向互通 |
...
- **2026-06**:
  ...
  - **连接管理系统全面升级**: 公共 ConnectionManager 组件，全驱动统一连接管理
