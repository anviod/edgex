...
1. **驱动采集流程**：
   - 驱动 → Points Layer → Shadow Ingress → Shadow Core.WriteShadowDevice → 通知订阅者

2. **北向控制流程**：
   - Northbound Adapter → Shadow Core.WriteShadowPoint → 通知订阅者

3. **RTT 优化流程**：
   - 驱动 → Shadow Core.UpdateDeviceRTT → ShadowDeviceOptimizer → 更新通信画像
...
### 5.2 可靠性机制
- **重启恢复**：配置从 `config.db` 恢复；影子实时态由 ScanEngine 重新采集填充
- **数据校验**：定期一致性检查
- **错误处理**：完善的错误处理和重试机制
- **审计日志**：记录所有操作日志
...
### 6.3 存储优化
- **内存存储**：实时数据纯内存存储，无磁盘持久化
- **设备历史快照**：由 `DeviceStorageManager` 从影子全量点位落盘（有界保留）
...
### 9.1 部署方式
- **裸机部署**：单二进制直接运行（推荐）
- **systemd 服务**：生产环境标准方式，见 [产品说明 — 部署流程](../guide/产品说明.html#部署流程) 与 [用户手册 — 部署流程](../guide/USER_MANUAL.html#部署流程)
- **嵌入式边缘设备**：ARM/x86 网关，最低 128MB 内存、1GB 存储

### 9.2 运维监控

零外部依赖（无 Prometheus/Grafana），通过 HTTP diagnostics + UI 轮询 + 结构化日志完成巡检。详见 [用户手册 — 运维诊断与 SLA 监控](../guide/USER_MANUAL.html#运维诊断与-sla-监控)。

| 通路 | 机制 | 入口 |
|------|------|------|
| **读** | REST diagnostics JSON | `GET /api/diagnostics/scan-engine` |
| **判** | 内置 `sla_warnings[]` 阈值 | 同上 + `GET /api/devices/:deviceId/diagnostics` |
| **告** | zap WARN 日志 + 通道 Event Log | 日志 `grep '[SLA]'` + `GET /api/channels/:channelId/diagnostics/events` |
| **看** | UI 指标页 | 首页 Soak 面板 · 通道列表 →「监控」弹窗 |

**Diagnostics 端点（均需 JWT）：**

- `GET /api/diagnostics/scan-engine` — ScanEngine 调度指标与 `sla_warnings[]`
- `GET /api/diagnostics/soak` — Soak 会话与 Release Gate
- `GET /api/devices/:deviceId/diagnostics` — 单设备 IO 画像、scan task lag、断路器状态
- `GET /api/channels/:channelId/diagnostics/events` — 通道 Event Log（CB Open/Reject 等）

**UI 指标页：**

- **首页**（`/dashboard`）：ScanEngine Soak 面板，展示 SLA / soak 运行态
- **通道列表** → 通道卡片「监控」：通道级 KPI（成功率、RTT、丢包）+ **调度 SLA**（Lag P95、CB Open、反压拒绝、`sla_warnings` 列表）
- **点位列表** → 点位「调试」：`GET /api/points/:pointId/debug` 查看原始字节与解析值
- 设备/通道 metrics API（`GET /api/channels/:id/metrics`、`.../devices/:id/metrics`）供 UI 与脚本调用

**结构化日志**：`grep '[SLA]'` 或 `grep 'circuit_breaker_open'` 于本地 app.log，无需外部日志栈。
