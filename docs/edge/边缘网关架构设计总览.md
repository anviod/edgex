...
**定位**：Mini OS Scheduler — 统一掌控时间（10ms Tick）、资源、执行分发与 ScanTask 状态机。调度闭环：`dispatch → Execute → WriteShadow → updateTaskState → 重新入堆`。
...
> 结构化状态快照见 [ROADMAP.md](../ROADMAP.html)；工程铁律见 [DEVELOPMENT_PRINCIPLES.md](../DEVELOPMENT_PRINCIPLES.html)。
