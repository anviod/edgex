...
- **Diagnostics**：复用 `/diagnostics/*` 与 UI 指标页
...
1. **单机部署**：在单个边缘设备上以裸机二进制或 systemd 服务运行
2. **分布式部署**：在多个边缘网关上分布式部署
3. **混合部署**：边缘设备与云端协同部署
4. **生产部署**：systemd 服务、防火墙与配置初始化，见 [产品说明 — 部署流程](../guide/产品说明.html#部署流程) 与 [README 部署流程](https://github.com/anviod/edgex#部署流程)
...
- **Diagnostics 集成**：复用 `/diagnostics/*` 与 UI 指标页
...
- **Diagnostics 巡检**：`GET /diagnostics/*` 与 `sla_warnings[]`
- **配置验证**：验证流程配置的正确性
- **状态恢复**：恢复系统的运行状态
- **批量操作**：批量管理多个流程
