...
| 群控策略 | 顺序启停、批量控制 |
| 数据聚合 | 多源求和、滑动平均、虚拟点位 |
| 其他 | 跨设备数据聚合、多设备联动控制、边缘实时决策、环境温湿度监控、预测性维护、产线节拍统计、设备远程监控等（8 个模版） |

UI 场景模版筛选栏为 **全部场景 / 告警联动 / 群控策略 / 数据聚合 / 其他**。除「告警联动」外，其余 8 个模版 `category` 均为「其他」；「群控策略」「数据聚合」Tab 通过 `sceneType` 交叉筛选对应模版。卡片标签展示细粒度 `sceneType`。
