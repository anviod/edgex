...
- **嵌入式友好**：影子设备纯内存态，不做 WAL；掉电后由采集引擎重建实时态，历史数据由 `DeviceStorageManager` 从影子全量快照落盘保留。

> 与早期设计差异：工业网关上不再追求「影子 WAL 零丢数」，而采用「内存实时 + 有界历史快照 + 配置强持久」分层策略，详见 `docs/operations/armv7-industrial-storage-optimization.md`。
...
| `ScanEngine` | 采集结果写入影子 | `internal/core/scan_engine.go` |
| `DeviceStorageManager` | 从影子读取全量点位，写入 `device_history_*`（每设备最多 1000 条） | `internal/core/device_storage_manager.go` |
| `ChannelManager` | `GetShadowPoint` / `getDevicePointsFromShadow` / 写点回写影子 | `internal/core/channel_manager.go` |
...
sequenceDiagram
    ...
    participant ShadowCore
    participant ShadowBridge
    participant Pipeline
    participant UI as UI/WebSocket
    participant NB as Northbound
    participant Hist as DeviceStorageManager

    Driver->>ScanEngine: 采集结果
    ScanEngine->>ShadowCore: WriteShadowDevice(msg)
    ShadowCore->>ShadowCore: 更新内存点位 + 通信画像
    ShadowCore-->>ShadowBridge: Subscribe 回调
    ShadowBridge->>Pipeline: PushBatch(values)
    Pipeline->>NB: handleValue
    Pipeline->>Hist: handleValue（realtime 策略时触发快照）
    ShadowCore-->>UI: Subscribe → BroadcastShadowPoint
    Hist->>Hist: saveSnapshot（从 ShadowCore 读全量点位）
```

步骤说明：

1. 驱动经 `ScanEngine` 完成采集。
2. `ScanEngine` 构造 `ShadowIngressMessage`，调用 `ShadowCore.WriteShadowDevice`。
3. 影子内存更新后，订阅者并行扇出：
...
> 说明：`ShadowIngress` 缓冲组件（`internal/core/shadow_ingress.go`）仍保留，支持 QoS 批量入队；当前生产主路径由 `ScanEngine` 直接写 `ShadowCore`，Ingress 可在高吞吐场景按需挂载。
...
目标：启动后尽快恢复采集与配置服务；不阻塞等待影子 WAL 回放。
...
**结论**：以 Shadow 为核心的实时数据面、北向共享、UI 更新、设备历史衍生等主路径**已满足**当前工业采集需求。文档早期 WAL / 零丢数 / ShadowIngress 必经等描述已过时，本章已同步修正。
