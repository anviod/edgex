...
| **EtherNet/IP (ODVA)** | ✅ 已实现 | 基于 `github.com/anviod/ethernet-ip` 库实现，完整支持 Rockwell Allen-Bradley PLC 全系列（ControlLogix、CompactLogix、Micro800、SLC 500、PLC-5）；支持 Tag 地址格式、多服务包（MSP）批量读取优化、CIP 消息通信、显式/隐式消息、自动重连及连接健康监控 |
| **Mitsubishi MELSEC (SLMP)** | 🚧 开发中 | 开发实现 |
| **Omron FINS (TCP/UDP)** | ✅ 已实现 | 基于 anviod/fins 库；TCP/UDP 双模式；CIO/D/W/H/EM 区域批量读写 |
| **SNMP v2c/v3** | ✅ 已实现 | Community / USM 认证；OID 批量采集；ScanObjects |
| **IEC 60870-5-104** | 🚧 M1 已交付 | 总召唤、自发上报、单点遥控；ConnectionManager 开发中 |
| **DL/T645-2007** | ⚠️ Stub | 已注册，模拟读写，真实协议开发中 |
| **Mitsubishi SLMP** | ⚠️ Stub | 已注册，模拟读写，真实协议开发中 |

> 驱动矩阵与测试报告：[docs/drivers/index.md](./drivers/index.md) · [docs/testing/南向驱动测试报告.md](./testing/南向驱动测试报告.md)
...
*   **配置管理**: 配置数据存储在 `data/config.db`（强一致写入），`data/runtime.db` 存储运行时数据（可清理/compact/重建）。
...
EdgeX 采用双数据库架构，配置与运行时数据分离存储：

*   `data/config.db`: **配置数据**（通道、设备、点位、用户、系统等，唯一权威配置数据源），强一致写入，优先备份，禁止清理
*   `data/runtime.db`: **运行时数据**（最新值、缓存、WAL、历史、规则状态），允许清理、compact、重建，不影响配置
*   `conf/`: 可选的 YAML 示例目录，运行时不再写入
...
> **数据库说明**：配置数据存储在 `data/config.db`，运行时数据存储在 `data/runtime.db`。不再使用 `edgex.db`，也不提供旧库迁移工具。
...
- [x] **EtherNet/IP**: 基于 `github.com/anviod/ethernet-ip` 库实现 CIP/EIP 协议栈。
- [ ] **其他驱动**: 逐步替换 Mitsubishi、DL/T645 的开发实现。（Omron FINS 已完成）
