...
layout: default
title: 南向采集数据通道质量优化
description: EdgeX 南向采集数据通道质量优化
---

# 南向采集数据通道质量优化

> **架构上下文**（2026-06）：南向采集由 **ScanEngine** 统一调度，设备状态经 `finalizeScanCollect` 回写；链路重连经 **ConnectionManager**。本文档通道级质量裁决与调度内核正交，可并行实施。
...
3️⃣ 在 StopChannel 或 ConnectionManager.ScheduleReconnect 中引入重连调度
...
功能	是否支持
设备在线/离线判定	✅ 原逻辑保留
通道通信质量等级	✅ 新增（Good/ Bad / Poor / Offline）
通道超时控制	✅ 支持
通道指数退避重连	✅ 支持
...
在 `finalizeScanCollect()`（ScanEngine 采集收尾）中，在你已有：
