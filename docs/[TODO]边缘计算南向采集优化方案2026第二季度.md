...
| ShadowCore | 内存 + WAL | ✅ | `shadow_core.go`；启动 WAL 恢复；5min 压实 |
...
| **ShadowCore 接入 ScanEngine** | `main.go` SetShadowCore | 采集 → 影子 → WAL |
...
```text
ScanEngine → Driver.ReadPoints → ShadowCore.WriteShadowDevice → WAL + WebSocket
                                      ↓ (断层)
                              DataPipeline ✗
                                      ↓
