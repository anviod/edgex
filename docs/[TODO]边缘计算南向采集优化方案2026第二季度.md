...
> **架构更新（2026-06-30）**：南向采集已迁移至 **ScanEngine 调度驱动内核**（`CollectionScheduler` / `deviceLoop` 已移除）。下文 `deviceLoop` / `collectDevice` / `CollectionScheduler` 等为 Q2 规划态；现行架构以 `docs/TODO/ScanEngine重构方案.md` 与 `docs/edge/边缘网关架构设计总览.md` 为准。
