...
### 11.2 开源 CS104 从站联调（推荐）

Freyr GUI 与 Python-IEC104（同厂商 DLL）在 M1 联调中均为 **握手通过、总召唤后读点失败**；CI 与 Windows 自动化请优先使用下列开源从站。

#### 11.2.1 方案对比

| 方案 | 仓库 | 许可 | Windows 构建 | M1 联调结论 |
| :--- | :--- | :--- | :--- | :--- |
| **go-iecp5 从站（Go 首选）** | [thinkgos/go-iecp5](https://github.com/thinkgos/go-iecp5) | GPL-3.0 | `go run`，无需 CMake | ✅ Connect + GI + M_ME_NA_1 IOA=1 **PASS** |
| **c104 Python 从站（Python 首选）** | [Fraunhofer-FIT-DIEN/iec104-python](https://github.com/Fraunhofer-FIT-DIEN/iec104-python)（PyPI: `c104`） | GPL-3.0 | `pip install c104` | ✅ Connect + GI + M_ME_NA_1 IOA=1 **PASS** |
| **lib60870 从站** | [mz-automation/lib60870](https://github.com/mz-automation/lib60870) | GPL-3.0 | 需 CMake + VS/MinGW | 源码已 vendoring，见 `test/lib60870/` |
| wendy512/iec104 | [wendy512/iec104](https://github.com/wendy512/iec104) | Apache-2.0 | Go（封装 go-iecp5） | 可选，与 go-iecp5 等价 |
| py_iec104 | [MaximPriyatkin/py_iec104](https://github.com/MaximPriyatkin/py_iec104) | MIT | 纯 Python，无依赖 | 未联测；项目较新，可作备选 |
| Freyr / pyiec104 DLL | 厂商闭源（`pyiec104` / `iec104x64d.dll`） | 试用 | 已测 | ❌ GI 后断连、无缓存、DLL 崩溃 |
| 仓库内 mock | `simulator_mock_integration_test.go` | — | 无外部依赖 | ✅ CI 默认 |

#### 11.2.2 go-iecp5 从站（Windows 一键）

**Headless 从站**（M_ME_NA_1 IOA=1 CA=1，归一化值 0.5）：

```bash
# 终端 1：启动从站（占用 2404）
cd test/ice104-goiecp5-server
go run . --bind 127.0.0.1 --port 2404

# 终端 2：联机测试
cd ../..
CGO_ENABLED=0 go test -tags=integration ./internal/driver/ice104/... \
  -run 'TestIntegrationWithGoIecp5Server|TestIntegrationWithSimulator|TestIntegrationWithMock104Server' -v -count=1
```

- `TestIntegrationWithGoIecp5Server`：测试进程内启动 go-iecp5 从站（随机端口），**不依赖 2404**，推荐 CI / 本地自动化。
- `TestIntegrationWithPythonServer`：测试进程内启动 c104 Python 从站（随机端口），需 `pip install c104`。
- `TestIntegrationWithSimulator`：探测 `127.0.0.1:2404` 是否有从站（Freyr / go-iecp5 / c104 手动启动均可）。

**驱动侧修复（2026-06-29）**：I 帧发送序号从 **0** 起算（原实现首帧 N(S)=1，与 go-iecp5 / lib60870 / c104 不兼容导致 GI 后断连）。

#### 11.2.3 c104 Python 从站（开源 Python 首选）

**选用理由**：在 PyPI / GitHub 检索 `iec60870`、`iec104`、`cs104 server python` 后，排除 Freyr `pyiec104`（ctypes 闭源 DLL，与 GUI 模拟器同故障）。**c104**（[iec104-python](https://github.com/Fraunhofer-FIT-DIEN/iec104-python)）为 Fraunhofer 维护的 **lib60870-C 官方 Python 绑定**（GPL-3.0），PyPI 标记 Production/Stable，Windows 有预编译 wheel，无需 CMake。

**备选**：`MaximPriyatkin/py_iec104` 为纯 Python 无依赖实现（MIT），项目较新未纳入联测；若无法安装 c104 wheel 可评估该库。

**安装与 headless 从站**（M_ME_NA_1 IOA=1 CA=1，归一化值 0.5）：
...
# 终端 2：联机测试
CGO_ENABLED=0 go test -tags=integration ./internal/driver/ice104/... \
  -run 'TestIntegrationWithPythonServer|TestIntegrationWithSimulator' -v -count=1
```

- `TestIntegrationWithPythonServer`：Go 测试自动 `exec` 启动 `server.py`（随机端口），**推荐 Python 侧自动化**；未安装 c104 时 `Skip`。
- 可通过环境变量 `PYTHON` 指定 Python 解释器路径。

#### 11.2.4 lib60870 从站（C，需构建）

源码与说明见 `test/lib60870/`：

- `edgex_server.c` — 最小 CS104 从站，响应总召唤返回 M_ME_NA_1 IOA=1
- `README.md` — clone lib60870 + CMake 构建步骤
- `build-windows.bat` — Windows 辅助脚本（需 PATH 中有 `cmake`）

构建并运行后：

```bash
./edgex_server --bind 127.0.0.1 --port 2404
CGO_ENABLED=0 go test -tags=integration ./internal/driver/ice104/... \
  -run TestIntegrationWithLib60870Server -v -count=1
```

可通过环境变量 `ICE104_LIB60870_ADDR=127.0.0.1:2405` 指定地址。

#### 11.2.5 EdgeX 点位配置（与 §11.1 相同）

| 配置项 | 值 |
| :--- | :--- |
| Group / typeID | **M_ME_NA_1** |
| Address / IOA | **1** |
| commonAddress | **1** |
| DataType | FLOAT 或 INT16 + Group=M_ME_NA_1 |

> **注意**：若仅选 **INT16** 而不指定 Group，驱动会默认映射到 **M_ME_NB_1 (TypeID=11)**，与模拟器 M_ME_NA_1 (9) 缓存键不一致，读点将始终为 Bad。
