...
> **架构对齐**（2026-06-27）：本驱动接入 Q3 统一采集架构 — `ChannelManager` 管理通道级 TCP 连接，`ScanEngine` 按 Scan Class 调度 `ReadPoints`，结果写入 `ShadowCore` 并经 `ShadowBridge` 扇出至 Pipeline。通道链路断开时，`channel_device_state.go` 将同通道设备标记为 Offline。详见 [TODO 规划索引](../index.md)。
