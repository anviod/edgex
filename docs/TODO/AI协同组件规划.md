...
> **架构结论**：**工业边缘自治 + AI 协同中心** — RK3588/3688 等边缘网关运行 EdgeX 采集内核、**报文捕获/解码**与 AI Agent Client；LLM/VLM/Embedding 与 **protocol_knowledge.db** 统一由远端 **AI Model Center** 承担；**AI 故障不得影响工业采集与规则执行**。
...
输入：`candidate_fields[]` + 监控表观测值（可选）+ `protocol_knowledge.db` 检索片段。  
...
知识沉淀：每次确认上线的映射写入 protocol_knowledge.db（§8）
```
...
**EdgeX Industrial Protocol Copilot** 采用 **边缘网关 + AI 推理中心分离** 架构。网关侧运行 **Capture / Decoder / Task Agent**（`internal/ai_agent/`）；文档解析、RAG、LLM 路由、**protocol_knowledge.db** 在远端 **AI Model Center** 完成。
...
| **AI Model Center** | AI Service（独立部署）     | 文档解析、Rule Engine、RAG、LLM 语义推理、**protocol_knowledge.db** | 直接写 `config.db`、调度 ScanEngine |
...
└── AI Model Center：PDF/OCR/RAG/LLM + protocol_knowledge.db
...
│ Capture/     │              │ knowledge.db + Rule Engine  │
│ Decoder/     │              │ Document Parser             │
│ Agent Client │              │                             │
└──────────────┘              └─────────────────────────────┘
```
...
（配置项、`ai_copilot` / `ai_task` / `ai_token_usage` bbolt 结构、用量统计、任务分级路由矩阵 — 与 V1.2 §3 保持一致，此处摘要。）
...
## §8 protocol_knowledge.db（协议知识库）



### 8.1 定位

**protocol_knowledge.db** 存储跨项目的协议模式与设备映射经验，随每次 Human Confirm 上线 **持续沉淀**。部署于 **AI Server**；可选同步只读缓存至网关（Rule Engine 离线识别增强）。
...
| 侧             | 路径                                               | 模式           |
| ------------- | ------------------------------------------------ | ------------ |
| **AI Server** | `data/protocol_knowledge.db`（SQLite）             | 读写；主库        |
| **网关（可选）**    | `runtime.db` → `protocol_knowledge_cache` bucket | 只读快照；定期 pull |
...
```sql
-- 协议帧模式
CREATE TABLE protocol_pattern (
  id            TEXT PRIMARY KEY,
  protocol_id   TEXT NOT NULL,          -- modbus-tcp | bacnet-ip | s7 | ...
  transport     TEXT,                   -- tcp | udp | serial
  port          INTEGER,
  header_magic  TEXT,                 -- mbap | bvlc | tpkt_cotp
  match_rules   JSON,                 -- Rule Engine 模式 JSON
  confidence_prior REAL DEFAULT 0.8,
  created_at    TEXT,
  updated_at    TEXT
);
...
CREATE TABLE device_model (
  ...
  default_params  JSON,               -- slave_id, byte_order, ...
  doc_ref         TEXT,
  project_count   INTEGER DEFAULT 0
);

-- 寄存器映射（沉淀自 confirm 的点位）
CREATE TABLE register_mapping (
  id              TEXT PRIMARY KEY,
  device_model_id TEXT REFERENCES device_model(id),
  point_semantic  TEXT,               -- Uab线电压 | 供水温度
  protocol_id     TEXT NOT NULL,
  ...
  unit            TEXT,
  confidence      REAL,
  source          TEXT,               -- doc-parse | protocol-reverse | manual
  evidence        TEXT,
  confirmed_at    TEXT,
  gateway_id      TEXT
);
...
-- 数据类型规则
CREATE TABLE datatype_rules (
  id            TEXT PRIMARY KEY,
  protocol_id   TEXT NOT NULL,
  raw_pattern   TEXT,                 -- 2_regs_float | 1_reg_uint16
  datatype      TEXT NOT NULL,
  byte_orders   JSON,                 -- ["ABCD","CDAB"]
  register_count INTEGER DEFAULT 1
);

-- 字节序规则
CREATE TABLE byte_order_rules (
  id            TEXT PRIMARY KEY,
  protocol_id   TEXT NOT NULL,
  manufacturer  TEXT,
  default_order TEXT NOT NULL,        -- ABCD | DCBA | ...
  alt_orders    JSON,
  notes         TEXT
);
...
CREATE TABLE conversion_rules (
  id            TEXT PRIMARY KEY,
  semantic_type TEXT,                 -- voltage | current | temperature | energy
  unit          TEXT,
  scale_candidates JSON,              -- [0.001, 0.01, 0.1, 1, 10]
  value_range   JSON,                 -- {"min": 0, "max": 500}
  protocol_id   TEXT
);
```
...
```text
Human Confirm → apply import
    ↓
register_mapping UPSERT + device_model.project_count++
    ↓
...
| **S20** | **protocol_knowledge.db 沉淀与检索**                                | **P1**     | S15 confirm 回流                      |
...
```text
文档片段 / 解码摘要 / knowledge.db 先例
    → Chunk + 协议标签（modbus_register_map · pcap_session · hmi_tag）
    → Embedding → 向量库
    → Top-K + 重排序 → 注入 LLM Prompt（仅阶段三语义推理）
```

冷启动语料：`docs/drivers/*`、`pointTemplates.json`、`protocol_knowledge.db` 种子数据。
...
⑤ 用户编辑 / 确认
⑥ apply → Channel + Point import + knowledge.db 沉淀
```
...
| **增强**  | 2026 Q4 | S6/S16/S20 + MQTT 弱网；BACnet 逆向；监控表多格式；knowledge.db 沉淀                                  | 3 项目试点；断网 `ai_task` 自动重投                                                                                |
| **企业级** | 2027 Q1 | S10/S17/S18：OCR、S7/EIP 逆向、Mode B 多网关                                                   | 100+ 网关共享 GPU Server                                                                                    |
| **持续**  | —       | knowledge.db 随项目增长；ROADMAP Phase 4 对齐                                                  | 月度 Token 成本报告                                                                                           |
...
| F6  | protocol_knowledge.db：confirm 后映射可检索并提升后续任务置信度                                     |
...
| 逆向误匹配            | 高   | 多观测联合 + knowledge.db 先例 + 人工并列候选 |
...
| `edgex-protocol-knowledge-query` | 检索 protocol_knowledge.db 先例              |
