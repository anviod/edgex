...
import (
	...
	"industrial-edge-gateway/internal/driver/bacnet/btypes/ndpu"
	"industrial-edge-gateway/internal/driver/bacnet/datalink"
	"industrial-edge-gateway/internal/driver/bacnet/encoding"
	"industrial-edge-gateway/internal/driver/bacnet/helpers/validation"
	"industrial-edge-gateway/internal/driver/bacnet/tsm"
	"industrial-edge-gateway/internal/driver/bacnet/utsm"
)
	...
	if header.Function == btypes.BacFuncBroadcast || header.Function == btypes.BacFuncUnicast || header.Function == btypes.BacFuncForwardedNPDU {
		...
		case btypes.UnconfirmedServiceRequest:
			if apdu.UnconfirmedService == btypes.ServiceUnconfirmedIAm {
				...
				err = dec.IAm(&iam)
				if err != nil {
					log.Printf("[ERROR] IAm decode error: %v", err)
					return
				}
				log.Printf("[DEBUG] Received IAM Message iam=%v from %v", iam.ID, src)
				iam.Addr = *src
				...
				log.Printf("[DEBUG] ignore who is request")
			} else {
				log.Printf("[ERROR] Unconfirmed: %d %v", apdu.UnconfirmedService, apdu.RawData)
			}
