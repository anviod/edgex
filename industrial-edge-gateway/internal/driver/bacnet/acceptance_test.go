...
func TestAcceptance_DeviceDiscovery(t *testing.T) {
	...
	if d.targetDevice.DeviceID != 1234 {
		t.Errorf("Expected DeviceID 1234, got %d", d.targetDevice.DeviceID)
	}
	if d.targetDevice.Vendor != 999 {
		t.Errorf("Expected VendorID 999, got %d", d.targetDevice.Vendor)
	}
	if d.targetDevice.MaxApdu != 1476 {
		t.Errorf("Expected MaxApdu 1476, got %d", d.targetDevice.MaxApdu)
	}
...
func TestAcceptance_ScanDeviceObjects(t *testing.T) {
	t.Run("ScanDeviceObjects", func(t *testing.T) {
		...
		d.targetDevice = btypes.Device{DeviceID: 1234}
		d.connected = true
...
func TestAcceptance_ObjectDiscovery(t *testing.T) {
	...
	d.targetDevice = btypes.Device{DeviceID: 1234}
	d.connected = true
	d.scheduler = NewPointScheduler(mockClient, d.targetDevice, 20, 10*time.Millisecond, 10*time.Second)
