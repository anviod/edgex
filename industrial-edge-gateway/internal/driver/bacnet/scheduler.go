...
func NewPointScheduler(client Client, targetDevice btypes.Device, groupThreshold uint16, instructionInterval time.Duration, cooldownDuration time.Duration) *PointScheduler {
	if groupThreshold == 0 {
...
func (s *PointScheduler) Read(ctx context.Context, points []model.Point) (map[string]model.Value, error) {
	...
	for i := 0; i < len(mpd.Objects); i += batchSize {
		...
		if err != nil {
			...
			// For now, let's record the first error but try to process others if possible,
			// or maybe just fail this chunk.
			// Current logic handles failure for "activePoints". We need to map chunk back to points.
			// This is complicated because "activePoints" is a flat list, but we grouped by Object.

			// Simple approach: Return error if any chunk fails, but maybe partial success is better?
			// Let's stick to simple failure for now, or log and continue.
			log.Printf("[WARN] BACnet Read chunk %d failed: %v", i/batchSize, err)
			if firstErr == nil {
				firstErr = err
			}
			// Mark associated points as failed?
			// We'd need to know which points are in this chunk.
			continue
		}
	...
	for _, obj := range resp.Objects {
		for _, prop := range obj.Properties {
			...
			if p, ok := pointMap[key]; ok {
				// Convert value based on point type or just use raw
				// The Data in Property is interface{}
				val := prop.Data

				// Optional: Perform type conversion if needed
