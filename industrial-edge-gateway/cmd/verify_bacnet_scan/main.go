...
func main() {
	...
	foundDevices := make(map[string]uint32) // ip->instance
	...
	for {
		...
		foundDevices[addr.IP.String()] = instance
	}
	...
	if len(foundDevices) == 0 {
		...
		// Read Object_Identifier (Prop 75) to get real ID
		// Then scan.
	...
	for ip, instance := range foundDevices {
		log.Printf("Scanning objects for device %d (%s)...", instance, ip)
		scanDeviceObjects(conn, ip, 47808, instance)
	}
}
