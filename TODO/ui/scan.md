以下是修改后的代码，在表头第一个位置增加了"全选"和"取消全选"按钮功能：
        ...
        <a-spin :loading="loading" class="w-full">
          ...
          >
            <template #headerCell="{ column }">
              <template v-if="column.dataIndex === 'selection' || column.slotName === 'selection'">
                <div class="selection-header">
                  ...
                  <div class="selection-actions">
                    <span class="select-all-text" @click="selectAllNew">全选</span>
                    <span class="select-divider">|</span>
                    <span class="clear-all-text" @click="clearAll">清空</span>
                  </div>
                </div>
              </template>
...
// 计算全选状态
watch([selected, filteredResults], () => {
  const selectable = selectableNodes.value.length
  selectAll.value = selectable > 0 && selected.value.length === selectable
...
const toggleSelectAll = (checked) => {
  ...
  if (checked) {
    selected.value = [...selectableNodes.value]
  } else {
    selected.value = []
  }
}
...
### 1. 表头选择区域增强
- 将列宽从 `48px` 增加到 `100px`，容纳复选框和文字按钮
- 添加 `selection-header` 容器，包含复选框和操作按钮
- 添加 `selection-actions` 区域，包含"全选"和"清空"文字按钮

### 2. 新增功能函数
- `selectableNodes`: 计算可选择的节点（仅新增节点）
- `selectAllNew()`: 全选所有新增节点
- `clearAll()`: 清空所有选中节点
- 优化了 `selectAll` 的自动计算逻辑

### 3. 交互优化
- 点击"全选"按钮：选中所有新增节点
- 点击"清空"按钮：清空所有选中节点
- 复选框状态与全选按钮联动
- 文字按钮有 hover 效果，提升交互体验

### 4. 样式优化
- 表头选择区域使用 flex 布局，复选框和文字按钮水平排列
- 文字按钮样式简洁，符合白色工业风
- 响应式设计，移动端自动调整为垂直排列